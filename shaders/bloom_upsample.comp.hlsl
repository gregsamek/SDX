...
SamplerState      sampler_small: register(s0, space0);
SamplerState      sampler_base: register(s1, space0);
...
void main(uint3 Gid : SV_DispatchThreadID)
{
    ...
    // 4 edges * 1
    c += small_level.SampleLevel(sampler_small, uv + float2(-2.0*o.x, 0.0), 0).rgb;
    c += small_level.SampleLevel(sampler_small, uv + float2( 2.0*o.x, 0.0), 0).rgb;
    c += small_level.SampleLevel(sampler_small, uv + float2(0.0, -2.0*o.y), 0).rgb;
    c += small_level.SampleLevel(sampler_small, uv + float2(0.0,  2.0*o.y), 0).rgb;

    // 4 diagonals * 2
    c += small_level.SampleLevel(sampler_small, uv + float2(-o.x, -o.y), 0).rgb * 2.0;
    c += small_level.SampleLevel(sampler_small, uv + float2( o.x, -o.y), 0).rgb * 2.0;
    c += small_level.SampleLevel(sampler_small, uv + float2(-o.x,  o.y), 0).rgb * 2.0;
    c += small_level.SampleLevel(sampler_small, uv + float2( o.x,  o.y), 0).rgb * 2.0;
    ...
    float3 base = base_level.SampleLevel(sampler_base, uv, 0).rgb;
